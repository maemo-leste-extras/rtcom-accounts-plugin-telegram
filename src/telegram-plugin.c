...
typedef struct _TelegramPluginClass TelegramPluginClass;
typedef struct _TelegramPlugin TelegramPlugin;

struct _TelegramPlugin
{
  RtcomAccountPlugin parent_instance;
};

struct _TelegramPluginClass
{
  RtcomAccountPluginClass parent_class;
};

ACCOUNT_DEFINE_PLUGIN(TelegramPlugin, telegram_plugin,
                      RTCOM_TYPE_ACCOUNT_PLUGIN);

static void
telegram_plugin_init(TelegramPlugin *self)
{
  RtcomAccountService *service;

  RTCOM_ACCOUNT_PLUGIN(self)->name = "telegram";
  RTCOM_ACCOUNT_PLUGIN(self)->capabilities =
      RTCOM_PLUGIN_CAPABILITY_ALL & ~RTCOM_PLUGIN_CAPABILITY_FORGOT_PWD & ~RTCOM_PLUGIN_CAPABILITY_REGISTER & ~RTCOM_PLUGIN_CAPABILITY_PASSWORD;

  /*
   * RTCOM_ACCOUNT_PLUGIN(self)->capabilities = RTCOM_PLUGIN_CAPABILITY_ALLOW_MULTIPLE |
...
create_advanced_settings_page(RtcomDialogContext *context)
{
  ...
  if (!dialog)
  {
    ...
    gchar title[200];
    const gchar *msg;
    GladeXML *xml = glade_xml_new(PLUGIN_XML_DIR "/telegram-advanced.glade",
...
static void
telegram_plugin_on_advanced_cb(gpointer data)
{
  GtkWidget *dialog;
...
static void
telegram_plugin_context_init(RtcomAccountPlugin *plugin,
                             RtcomDialogContext *context)
{
  gboolean editing;
  AccountItem *account;
  ...
  if (editing)
  {
    ...
    rtcom_edit_connect_on_advanced(
          RTCOM_EDIT(page), G_CALLBACK(telegram_plugin_on_advanced_cb),
          context);
  }
  else
  {
    gchar *username_label = g_strconcat(_("accounts_fi_telegram_user_and_host"),
                                        NULL);
    ...
    rtcom_login_connect_on_advanced(
          RTCOM_LOGIN(page), G_CALLBACK(telegram_plugin_on_advanced_cb),
          context);
  }
...
static void
telegram_plugin_class_init(TelegramPluginClass *klass)
{
  RTCOM_ACCOUNT_PLUGIN_CLASS(klass)->context_init = telegram_plugin_context_init;
}
